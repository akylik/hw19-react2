...
export default class Signin extends Component {
  ...
  componentDidMount = async () => {
    ...
    if (checkStor) {
      await this.setState({
        email: localStorage.getItem(this.state.prefixIn + "Email"),
      });
      await this.setState({
        password: localStorage.getItem(this.state.prefixIn + "Password"),
      });
  ...
  changeSignin = () => {
    ...
    if (this.state.checkBox) {
      ...
      localStorage.setItem(
        this.state.prefixIn + "Password",
        this.state.password
      );
      localStorage.setItem(
        this.state.prefixIn + "CheckBox",
        this.state.checkBox
      );
    } else {
    ...
    if (
      localStorage.getItem(this.state.prefixUp + "Email").toLowerCase() ===
        emailIn &&
      localStorage.getItem(this.state.prefixUp + "Password").toLowerCase() ===
        passIn
    ) {
  ...
  changeCheckBox = async (e) => {
    await this.setState({ checkBox: e.target.checked });
  };
  ...
  changeEmail = async (e) => {
    ...
    await this.setState({
      emailClr: checker >= 0 ? "green" : checker === -1 ? "red" : "",
    });
  ...
  changePassword = async (e) => {
    ...
    await this.setState({
      passClr: checker >= 0 ? "green" : checker === -1 ? "red" : "",
    });
