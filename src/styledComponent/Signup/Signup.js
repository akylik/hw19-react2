...
export default class Signin extends Component {
  ...
  componentDidMount = async () => {
    ...
    if (checkStor) {
      await this.setState({
        firstName: localStorage.getItem(this.state.prefix + "FirstName"),
      });
      await this.setState({
        lastName: localStorage.getItem(this.state.prefix + "LastName"),
      });
      await this.setState({
        email: localStorage.getItem(this.state.prefix + "Email"),
      });
      await this.setState({
        password: localStorage.getItem(this.state.prefix + "Password"),
      });
  ...
  changeFirstName = async (e) => {
    ...
    await this.setState({
      fNameClr: checker >= 0 ? "green" : checker === -1 ? "red" : "",
    });
  ...
  changeLastName = async (e) => {
    ...
    await this.setState({
      lNameClr: checker >= 0 ? "green" : checker === -1 ? "red" : "",
    });
